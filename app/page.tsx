...
function GalleryPreview() { const [galleryIndex, setGalleryIndex] = useState(0); const gallerySlides = [...slides, ...pathwayImages].slice(0, 10); useEffect(() => { const timer = window.setInterval(() => setGalleryIndex((current) => (current + 1) % gallerySlides.length), 4200); return () => window.clearInterval(timer) }, [gallerySlides.length]); const gallerySlide = gallerySlides[galleryIndex]; return <section className="gallery-preview" id="gallery"><div className="wide"><div className="gallery-heading"><span className="kicker purple">Explore school life</span><h2>Explore school life</h2></div><div className="media-grid"><div className="media-window"><span className="media-label">Watch Life At Laiser Hill</span><iframe title="Laiser Hill Academy video" src="https://www.facebook.com/plugins/video.php?href=https%3A%2F%2Fwww.facebook.com%2Freel%2F2213705062405863%2F&show_text=0&width=560" width="460" height="315" scrolling="no" frameBorder="0" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share" allowFullScreen /></div><div className="media-window gallery-window"><span className="media-label">Photo gallery</span><img className="gallery-slide" src={gallerySlide.src} alt={gallerySlide.alt} /><span className="gallery-count">0{galleryIndex + 1} / 10</span></div></div></div></section> }
